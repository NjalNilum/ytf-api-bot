...
namespace Tests
{
    ...
    public class YoutubeApiTests
    {
        ...
        /// <summary>
        /// Actual test. Each channel directory should now contain 10 dummy *.video.json files. The worker should fetch 2 real videos per channel
        ...
        /// only 200% of the number of videos fetched. Each channel directory may therefore contain 4 files.
        /// 
        /// Note that the rolling file appender will delete the oldest files in the working dir. For this reason, the videos that are read from
        /// the real channels are also deleted again, because all dummy videos have the time stamp of today and are therefore newer!  
        /// </summary>
        [TestMethod]
        public void StartYoutubeWorkerAndUpdateRollingFileTest()
        {
            ...
            var folder1 = VideoMetaDataFull.GetChannelSubDir(WorkFolder, theTestChannel.ChannelId);
            var folder2 = VideoMetaDataFull.GetChannelSubDir(WorkFolder, secondChannel.ChannelId);
            Assert.AreEqual(Directory.GetFiles(folder1).Length, 4);
            Assert.AreEqual(Directory.GetFiles(folder2).Length, 4);
        }
