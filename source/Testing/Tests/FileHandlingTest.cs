...
namespace Tests
{
    ...
    public class FileHandlingTest
    {
        ...
        public void TestTrimFile()
        {
            ...
            string iBimsATestFile = Path.Combine(WorkFolder, "testFile.txt");
            for (var i = 0; i < 56; i++)
            {
        ...
        public void TestFindNotYetProcessedVideoIdFiles()
        {
            ...
            var alreadyProcessed = Path.Combine(WorkFolder, mySubDir, $"{MakeVideoId(0)}.{VideoMetaDataFull.VideoFileSearchPattern}") + Environment.NewLine;
            alreadyProcessed += Path.Combine(WorkFolder, mySubDir, $"{MakeVideoId(1)}.{VideoMetaDataFull.VideoFileSearchPattern}") +Environment.NewLine;
